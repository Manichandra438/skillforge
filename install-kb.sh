...
SKILL_REL_PATH="plugins/kb/skills/kb"
...
if [ "$DO_CLAUDE" = "1" ]; then
  ...
  cp -r "$SRC/$SKILL_REL_PATH" "$HOME/.claude/skills/kb"
  echo "Claude Code: installed kb skill -> $HOME/.claude/skills/kb/SKILL.md"
fi

if [ "$DO_COPILOT" = "1" ]; then
  mkdir -p "$TARGET_DIR/.github/skills"
  cp -r "$SRC/$SKILL_REL_PATH" "$TARGET_DIR/.github/skills/kb"
  echo "Copilot CLI: installed kb skill -> $TARGET_DIR/.github/skills/kb/SKILL.md"
fi

echo "Done. See README.md for optional CLAUDE.md trigger/auto-load setup."
